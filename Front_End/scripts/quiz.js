(() => {
  ...
  async function grade() {
    ...
    if (window.saveQuizToCloud) {
      try {
        await window.saveQuizToCloud({ score: correct, total: questions.length });
        result.innerHTML += `<div class="card" style="margin-top:8px;color:#4f7cff">✅ Result saved to cloud!</div>`;
      } catch (err) {
