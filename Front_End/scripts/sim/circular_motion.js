  ...
  register({
    ...
    init(container) {
      ...
      function draw(p, d) {
        ...
        const cx = (W * 0.5) * zoomLevel + offsetX;
        const cy = (H * 0.55) * zoomLevel + offsetY;
        const pxPerM = Math.min((W * 0.35) / p.r, (H * 0.45) / p.r) * zoomLevel;
        ...
        ctx.beginPath(); ctx.fillStyle = "#1f2937"; ctx.arc(x, y, 7 * zoomLevel, 0, Math.PI * 2); ctx.fill();


        ctx.beginPath(); ctx.moveTo(x, y);
        ctx.lineTo(x + 30 * zoomLevel * (-Math.sin(theta)), y + 30 * zoomLevel * (Math.cos(theta)));
        ...
        ctx.beginPath(); ctx.moveTo(x, y);
        ctx.lineTo(x + 40 * zoomLevel * (ux / ulen), y + 40 * zoomLevel * (uy / ulen));
